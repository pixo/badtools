...
import badass.core
import badass.utils
import badtools.utils as utils
...
class UiPush (QtGui.QWidget):
    ...
    def createWidgetList(self):
        self.workspace = badass.core.getPathFromId(self.docId, True)
...
class UiCreateAsset(QtGui.QWidget):
    ...
    def __init__(self, parent=None, assetManager=None):
        ...
        self.comboBoxAssetTypeFilter.setMaximumSize(
        ...
        self.assetTypes = badass.utils.getAssetTypes()
    ...
    def setUi(self):
        # Title
        title = "<html><head/><body><p><span style=\" font-size:12pt; "
        title += "font-weight:600;\">%s</span>" % ("Create Asset")
        title += "<span style=\" font-size:12pt;\"/></body></html>"
...
class UiCreateTask (QtGui.QWidget):
    ...
    def __init__(self, parent=None, db=None, item=None, assetManager=None):
        ...
        self.assetManager = assetManager
        self.item = item
        self.tasks = badass.core.lsDb(self.db, "task", self.item.id)
        self.taskTypes = badass.utils.getAssetTasks()
    ...
    def setUi(self, Form):
        Form.setWindowTitle(QtGui.QApplication.translate(
            "Form", "create task(s)", None, QtGui.QApplication.UnicodeUTF8))
...
class UiAssetManager(QtGui.QMainWindow):
    ...
    def createUi(self):
        ...
        self.comboBoxAssetTaskFilter.setMaximumSize(
            QtCore.QSize(16777215, 16777215))
        ...
        self.comboBoxAssetForkFilter.setMaximumSize(
            QtCore.QSize(16777215, 16777215))
        ...
        # Workspace button
        self.buttonCreateWorkspaceToolBar = QtGui.QPushButton(
            self.centralwidget)
        ...
        # Delete asset
        self.buttonDeactivateAssetToolBar = QtGui.QPushButton(
            self.centralwidget)
        ...
        self.buttonDeactivateAssetToolBar.setObjectName(
            "buttonDeactivateAssetToolBar")
        ...
        self.assetTypes = badass.utils.getAssetTypes()
        self.assetTypes = dict(
            {"No filter": "asset"}.items() + self.assetTypes.items())
        self.assetTypesSwap = dict((v, k)
                                   for k, v in self.assetTypes.iteritems())
        self.assetTasks = badass.utils.getAssetTasks()
        self.assetTasks = dict(
            {"No filter": "task"}.items() + self.assetTasks.items())
        self.assetTasksSwap = dict((v, k)
                                   for k, v in self.assetTasks.iteritems())
        self.allTasks = dict()
    ...
    def createFilterTypes(self):
        ...
        for couple in items:
            icon = utils.getIconPath(
                "asset" if couple[0] == "No filter" else couple[0])
            self.comboBoxAssetTypeFilter.addItem(QtGui.QIcon(icon), couple[0])
    ...
    def createFilterTasks(self):
        ...
        for couple in items:
            icon = utils.getIconPath(
                "task" if couple[0] == "No filter" else couple[0])
    ...
    def createTree(self):
        ...
        for asset in self.allAssets:
            ...
            if values["inactive"]:
                assetItem.setBackground(
                    0, QtGui.QBrush(QtCore.Qt.red, QtCore.Qt.Dense4Pattern))
    ...
    def refreshTasks(self):
        """Refresh"""
        self.allTasks = badass.core.lsDb(self.db, "task", self.project)
        self.taskClicked()

    def refreshTree(self):
        self.allAssets = badass.core.lsDb(self.db, "asset", self.project)
    ...
    def createTask(self):
        ...
        if not item or not hasattr(item, 'id'):
            return
        self.createTaskWidget = UiCreateTask(
            db=self.db, item=item, assetManager=self)
    ...
    def workspace(self):
        item = self.treeWidgetMain.currentItem()
        badass.core.createWorkspace(item.id, "review")
    ...
    def pullClicked(self):
        ...
        self.statusbar.showMessage(
            "Pulling asset '%s' version '%s'" % (docId, version))

        """Pull asset"""
        badass.core.pull(db=self.db, doc_id=docId, version=version,
    ...
    def assetClicked(self):
        ...
        # Enable buttons
        stat = False
        self.buttonCreateAssetToolBar.setDisabled(stat)
        ...
        self.buttonDeactivateAssetToolBar.setIcon(
            QtGui.QIcon(utils.getIconPath(icon)))
    ...
    def taskClicked(self):
        ...
        for version in versions:
            versionsList.append("%03d" % int(version))
    ...
    def setDescription(self, item, doc):
        ...
        if doc:
            ...
            creator = "Creator:\t%s\n" % doc[
                'creator'] if ('creator' in doc) else ''
            created = time.localtime(float(doc['created'])) if (
    ...
    def releaseAsset(self):
        ...
        msgBox = QtGui.QMessageBox()
        msgBox.setText(
            "Are you sure to release %s version %s ?" % (slug, version))
        msgBox.setStandardButtons(
            QtGui.QMessageBox.Ok | QtGui.QMessageBox.Cancel)
        msgBox.setDefaultButton(QtGui.QMessageBox.Save)
        ret = msgBox.exec_()
        ...
        if ret == QtGui.QMessageBox.Ok:
            badass.core.release(self.db, item.id, version)
            self.refreshTree()
        else:
    ...
    def signalConnect(self):
        self.comboBoxAssetTypeFilter.currentIndexChanged.connect(
            self.filterAssets)
        self.comboBoxAssetTaskFilter.currentIndexChanged.connect(
            self.filterTasks)
        self.comboBoxAssetForkFilter.currentIndexChanged.connect(
            self.filterTasks)
        self.comboBoxVersionType.currentIndexChanged.connect(
            self.versionTypeChanged)
    ...
    def itemExpanded(self, item):
        tasks = badass.core.lsDb(self.db, "task", item.id)
        if len(tasks) == 0:
            return
        item.takeChildren()
    ...
    def setUi(self):
        ...
        self.labelFilter.setToolTip(
            "Filter elements by asset <b>types and tasks</b>.")

        # Search
        self.labelSearch.setPixmap(utils.getIconPath("search"))
        self.labelSearch.setToolTip(
            "Search elements with <b>part of name</b>.")
        ...
        self.buttonCreateWorkspaceToolBar.setIcon(
            QtGui.QIcon(utils.getIconPath("workspace")))
        self.buttonCreateWorkspaceToolBar.setToolTip(
            "Create <b>Workspace</b>.")

        # Create Asset button
        self.buttonCreateAssetToolBar.setIcon(
            QtGui.QIcon(utils.getIconPath("addasset")))
        self.buttonCreateAssetToolBar.setToolTip("Create a new <b>Asset</b>.")

        # Create Task button
        self.buttonCreateTaskToolBar.setDisabled(True)
        self.buttonCreateTaskToolBar.setIcon(
            QtGui.QIcon(utils.getIconPath("addtask")))
        self.buttonCreateTaskToolBar.setToolTip(
            "Add <b>Task(s)</b> to selected Asset.")

        # Push button
        self.buttonPushToolBar.setIcon(QtGui.QIcon(utils.getIconPath("push")))
        self.buttonPushToolBar.setToolTip(
            "<b>Push</b> a task to the repository.")

        # Pull button
        self.buttonPullToolBar.setIcon(QtGui.QIcon(utils.getIconPath("pull")))
        self.buttonPullToolBar.setToolTip(
            "<b>Pull</b> a task from the repository.")

        # Set status button
        self.buttonSetStatusToolBar.setIcon(
            QtGui.QIcon(utils.getIconPath("statuses")))
        self.buttonSetStatusToolBar.setToolTip("Set <b>Status</b>.")

        # Set delete/inactive button
        self.buttonDeactivateAssetToolBar.setIcon(
            QtGui.QIcon(utils.getIconPath("disconnect")))
        self.buttonDeactivateAssetToolBar.setToolTip(
            "<b>activate/deactivate</b> an asset.")

        # Set release button
        self.buttonReleaseToolBar.setIcon(
            QtGui.QIcon(utils.getIconPath("release")))
